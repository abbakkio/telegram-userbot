...
def setup(client: TelegramClient):
    @client.on(events.NewMessage(outgoing=True, pattern=r'(?i)^\.roll(?:\s+(\d+)-(\d+))?$'))
    async def roll_handler(event):
        try:
            ...
            for i in range(frames_count):
                ...
                try:
                    await event.edit(f"{emoji} **Rolling:** {temp_num}")
                    await asyncio.sleep(0.2)
                except MessageNotModifiedError:
                    continue # Ignore if the random number happened to be the same as the last frame
                    
            # Set the final result
            await event.edit(f"🎯 **Result:** {final_result}  *(Range: {min_val}-{max_val})*")
